...
impl CPU {
    ...
    fn RES(&mut self, bit:u8, num:u8) -> u8 {
        num & !(1 << bit)
    }

    fn SET(&mut self, bit:u8, num:u8) -> u8{
        num | (1 << bit)
    }

    fn BIT(&mut self, bit:u8, num:u8){
        let res = (num & (1 << bit)) == 0;

        self.update_flags(res, self.get_CF(), true, false);
    }

    fn DAA(&mut self) { // Fumada que no se usa nunca. Generated by Chat-GPT
